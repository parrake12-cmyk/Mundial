...
function renderHeroStats(totals) {
  const completed = state.fixtures.filter((fixture) => hasResolvedResult(fixture.matchNumber, state.results, state.liveScores)).length;
...
function renderMatchCard(fixture) {
  ...
  const result = resolveMatchResult({ matchNumber: fixture.matchNumber, stateResults: state.results, liveScores: state.liveScores });
  const display = result || live || {};
  const done = Boolean(result || live?.finished);
...
function renderGroups() {
  const groups = buildGroupStandings(state.fixtures, state.results, state.liveScores);
...
function scoringResult(matchNumber) {
  return resolveMatchResult({ matchNumber, stateResults: state.results, liveScores: state.liveScores });
}

function outcomeFor(home, away) {
  return getMatchOutcome(home, away);
}

function hasResult(matchNumber) {
  return hasResolvedResult(matchNumber, state.results, state.liveScores);
}
